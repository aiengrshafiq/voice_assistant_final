...
from app.services.speech_service import speak, speak_translation, StableSpeechToText, FastSpeechToText
...
class VoiceAssistantStateMachine:
    def __init__(self):
        self.audio_stream = AudioStreamService(frame_length=512, sample_rate=16000)
        self.wake_word_engine = WakeWordEngine(audio_stream=self.audio_stream)
        
        if settings.USE_EXPERIMENTAL_STT:
            logger.info("🚀 EXPERIMENTAL FAST MODE ENABLED: Using Deepgram STT.")
            self.stt_service = FastSpeechToText(audio_stream=self.audio_stream)
        else:
            logger.info("✅ STABLE MODE ENABLED: Using Google STT.")
            self.stt_service = StableSpeechToText()
            
        self.state = AssistantState.IDLE
        self.nlu_result: dict | None = None

    def run(self):
        logger.info("Definitive V4.2 State Machine started.")
        try:
    ...
    async def _handle_listening_state(self):
        command = await self.stt_service.listen()
        if command:
    ...
    async def _handle_processing_state(self):
        ...
        if self.nlu_result.get('intent') == 'start_translation':
            ...
            await asyncio.to_thread(speak, "I'm not sure how to handle that.")
            self.state = AssistantState.IDLE
    ...
    async def _handle_awaiting_confirmation_state(self):
        ...
        confirmation_command = await self.stt_service.listen()
        if await is_confirmed(confirmation_command):
            self.state = AssistantState.EXECUTING
        else:
            if confirmation_command:
                self.nlu_result = await detect_intent_with_context(confirmation_command, [])
                self.state = AssistantState.PROCESSING
            else:
    ...
    async def _handle_executing_state(self):
        ...
        if message: await asyncio.to_thread(speak, message)
        if result.get("status") == "conflict": self.state = AssistantState.AWAITING_CONFIRMATION
        else: self.state = AssistantState.IDLE
    ...
    async def _handle_translating_state(self):
        ...
        while self.state == AssistantState.TRANSLATING:
            text_to_translate = await self.stt_service.listen(timeout=30)
            ...
            if "stop translation" in text_to_translate.lower():
                ...
                self.state = AssistantState.IDLE; break
            ...
            if translated_text: await asyncio.to_thread(speak_translation, translated_text, lang=target_lang)
            else: await asyncio.to_thread(speak, "Sorry, I could not get a translation.")